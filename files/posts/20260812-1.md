...
title: "macOS에서 안드로이드 스마트폰 사진·파일 전송법: OpenMTP 설치부터 활용까지 완벽 가이드"
date: "2026-08-12"
...
3. 다운로드한 `.dmg` 파일을 더블클릭하고, **OpenMTP 아이콘을 Applications(응용 프로그램) 폴더로 드래그**하여 설치를 완료합니다.
...
앱을 처음 실행할 때 **"개발자를 확인할 수 없기 때문에 열 수 없습니다"**라는 경고창이 나타날 수 있습니다.
* <strong>해결 방법:</strong> macOS의 **'시스템 설정(System Settings) ➔ 개인정보 보호 및 보안(Privacy & Security)'**으로 이동하여 하단의 **[확인 없이 열기(Open Anyway)]** 버튼을 클릭하거나, 응용 프로그램 폴더에서 OpenMTP 앱을 `Control` 키를 누른 채 클릭 후 **[열기]**를 선택합니다.

---

## 📱 3. 안드로이드 스마트폰 연결 및 USB MTP 모드 설정

OpenMTP 앱을 설치했다고 끝이 아닙니다. 안드로이드 스마트폰과 맥북을 USB 케이블로 연결할 때 스마트폰에서 **파일 전송 권한**을 승인해 주어야 합니다.
...
3. 용도를 **'파일 전송 / Android Auto'** 또는 **'MTP(미디어 전송 프로토콜)'**로 선택합니다.
4. 스마트폰 화면에 **"휴대폰 데이터에 접근하도록 허용하시겠습니까?"** 팝업이 뜨면 **[허용]**을 누릅니다.
...
OpenMTP를 실행하면 왼쪽 영역은 **내 맥(Mac)의 데스크톱/다운로드/사진 폴더**가 표시되고, 오른쪽 영역은 **안드로이드 폰의 내부 저장소(Internal Storage) 및 SD카드**가 자동 표시됩니다.
...
* <strong>안드로이드 ➔ 맥 전송:</strong> 우측 스마트폰 `DCIM/Camera` 폴더에서 옮기고 싶은 사진이나 동영상 파일을 선택한 후, 좌측 맥의 원해는 폴더(예: `Pictures` 또는 `Downloads`)로 **드래그 앤 드롭(Drag & Drop)**합니다.
...
* <strong>해결책:</strong> OpenMTP 설정(Gear 아이콘) ➔ `MTP Mode` 항목에서 기본값인 **'Kagami Kernel MTP'** 모드를 **'Legacy-MacUSB MTP'** 모드로 변경한 후 앱을 재시작하면 호환성 문제가 깔끔히 해결됩니다.
...
**OpenMTP**는 **애플 실리콘 최적화 고속 전송 ➔ 듀얼 페인 드래그 앤 드롭 ➔ 삼성 스마트 스위치 충돌 자동 차단**까지 갖춘 맥 사용자 필수 꿀 유틸리티입니다. 오늘 소개해 드린 가이드를 바탕으로 스마트폰 안의 소중한 사진과 동영상을 맥북으로 손쉽게 백업해 보세요!
